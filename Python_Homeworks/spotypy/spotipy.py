...
class SpotiPy:
    ...
    def fromStringToArtist(artistString):
        ...
        listOfUsefulValues = artistString.split(',',3)
        name=listOfUsefulValues[0]
        lastName=listOfUsefulValues[1]
        birthYear=listOfUsefulValues[2]
        albumsAndSingles = listOfUsefulValues[3].split("singles:")
        albumsString = albumsAndSingles[0].strip()
        albumsString = albumsString[8:]
        albums=SpotiPy.fromStringToAlbum(albumsString)
        singlesString=albumsAndSingles[1].strip()
        while singlesString[0]=="{":
            singlesString=singlesString[1:]
            singlesString=singlesString.strip()
        singles=[]
...
# x = SpotiPy.loadFromFile("data/data2.txt")
...
# a= SpotiPy.fromStringToSong("")
# print(a)
# a = SpotiPy.fromStringToArtist("    Jarrett,Church,1981,    albums:    {        Shortsighted Client,2018,songs:        {             Each Panics,1.2 minutes,2005,175            |Petri Stiller,0.5 minutes,2012,135        }    },    singles:    {        Winer Retarder Florin,3.1 minutes,2018,109    }")
# print(a)
# print(SpotiPy.fromStringToAlbum(" {        Shortsighted Client,2018,songs:        {             Each Panics,1.2 minutes,2005,175            |Petri Stiller,0.5 minutes,2012,135        } ")[0])
